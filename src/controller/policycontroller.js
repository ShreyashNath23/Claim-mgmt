...
module.exports = {
  createPolicy: async (req, res) => {
    ...
    return res.status(201).json(Array.from(policy.values()));
  },
  ...
  getPolicy: (req, res) => {
    ...
    if (req.body.policyid) {
      ...
      return res.status(200).json(policyInfo);
    } else {
      return res.status(200).json(Array.from(policy.values()));
    }
  },
  ...
  updatePolicy: (req, res) => {
    ...
    return res.status(200).json(updatedPolicyInfo);
  },
  ...
  deletePolicy: (req, res) => {
    if (policy.has(req.body.policyid)) {
      ...
      return res.status(401).json("Sorry, this policy does not exist");
    }
  },
};
